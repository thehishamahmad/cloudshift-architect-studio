...
function generateRecommendation(inputs) {
  ...
  const normalizedConcurrentUsers = Math.max(0, Number(concurrentUsers) || 0);
  const normalizedDataTransfer = Math.max(0, Number(dataTransfer) || 0);
  const isHighConcurrency = normalizedConcurrentUsers >= 3000;
  const isVeryHighConcurrency = normalizedConcurrentUsers >= 10000;
  ...
  const recommendsCloudArmor = isHighSecurity || isStrictSecurity || workloads.includes('gov-portal');
  ...
  if (isMultiRegion) {
    if (isLegacyOrVM) {
      computeService.name = 'Compute Engine (Multi-Region Managed Instance Groups)';
      computeService.rationale = 'Runs independently autoscaled VM groups in two regions behind global load balancing for mission-critical legacy workloads.';
    } else if (isStateful || isVeryHighConcurrency) {
      computeService.name = 'GKE Standard (Multi-Region Autoscaling Clusters)';
      computeService.rationale = 'Runs independent regional GKE clusters with autoscaling node pools, suitable for stateful orchestration or very high concurrency.';
    } else {
      computeService.name = 'Cloud Run (Multi-Region)';
      computeService.rationale = 'Deploys the stateless service to at least two regions behind global load balancing, with per-region autoscaling and health-based failover.';
    }
    ...
    } else if (isHighBudget || recoveryTier === 'tier-2' || isHighConcurrency) {
  ...
  if (computeService.name.includes('GKE')) {
    const baselinePods = Math.max(2, Math.ceil(normalizedConcurrentUsers / 500));
    computeService.rationale += ` Initial sizing assumes approximately ${baselinePods} application pods at 500 concurrent users per pod, validated by load testing.`;
  } else if (computeService.name.includes('Cloud Run')) {
    const maxInstances = Math.max(2, Math.ceil(normalizedConcurrentUsers / 80));
    computeService.rationale += ` Start with concurrency 80 and a max-instance guardrail near ${maxInstances}, then tune from observed latency and CPU.`;
  } else if (computeService.name.includes('Instance Groups')) {
    const baselineInstances = Math.max(2, Math.ceil(normalizedConcurrentUsers / 500));
    computeService.rationale += ` Plan an initial cross-zone or cross-region baseline of approximately ${baselineInstances} instances, subject to load testing.`;
  }
  ...
  if (workloads.includes('dwh') || workloads.includes('analytics')) {
    ...
    if (isMultiRegion) {
      ...
      dbService.rationale = 'Multi-region relational database for mission-critical transactional workloads, providing synchronous replication and strong consistency across regions.';
    ...
    // NoSQL defaults
    if (isMultiRegion) {
      dbService.name = 'Cloud Firestore (Multi-Region)';
      dbService.rationale = 'Serverless multi-region document database with automatic scaling and regional redundancy for non-relational mission-critical workloads.';
    } else if (isLowBudget) {
  ...
  if (workloads.includes('streaming') || workloads.includes('lms') || isHighTransfer) {
    storageService.name = 'Cloud Storage (Multi-Regional Standard)';
    storageService.rationale = `Uses the Standard storage class for frequently accessed content and multi-region placement for resilient delivery of approximately ${normalizedDataTransfer.toLocaleString()} GB/month.`;
  } else if (isMultiRegion) {
    storageService.name = 'Cloud Storage (Multi-Regional Standard)';
    storageService.rationale = 'Uses Standard-class, geographically redundant object storage to support regional failover and frequently accessed production data.';
  } else if (isHighAvailability) {
    ...
    storageService.rationale = 'Standard-class regional storage supports active production objects without multi-region replication cost.';
  }
  ...
  if (isMultiRegion && isInternalOnly) {
    netService.name = 'Global HTTPS Load Balancer & Private Service Connect';
    netService.rationale = 'Provides global health-based routing across regional backends while Private Service Connect restricts application access to approved internal networks.';
  } else if (isMultiRegion) {
    netService.name = shouldUseCdn ? 'Global HTTPS Load Balancer & Cloud CDN' : 'Global HTTPS Load Balancer';
    netService.rationale = shouldUseCdn
      ? `Routes users to healthy regional backends and caches eligible content at the edge to reduce origin load and transfer cost for ${normalizedDataTransfer.toLocaleString()} GB/month.`
      : 'Routes requests to healthy backends across regions and provides automatic regional failover.';
  ...
  if (recoveryTier === 'tier-1') {
    rtoValue = '< 15 Min';
    rpoValue = '< 1 Min';
    const computeDR = computeService.name.includes('GKE')
      ? `${computeService.name} runs independent clusters in separate regions.`
      : computeService.name.includes('Cloud Run')
        ? `${computeService.name} runs the service in separate regions with health-based routing.`
        : `${computeService.name} runs independent managed groups in separate regions.`;
    const dataDR = dbService.name === 'Cloud Spanner'
      ? 'Cloud Spanner provides multi-region synchronous replication for relational transactions.'
      : dbService.name.includes('Firestore')
        ? `${dbService.name} provides managed multi-region replication for document data.`
        : `${dbService.name} remains the selected workload-specific data service and must use its supported multi-region configuration.`;
    recoveryDR = `Designed for mission-critical operations. ${computeDR} ${dataDR} ${netService.name} performs health-based regional failover, while ${storageService.name} keeps object data geographically redundant. Recovery targets must be validated through scheduled failover exercises.`;
  ...
  if (isHighTransfer) {
    costRecs.push(`**Network Egress Controls:** At approximately ${normalizedDataTransfer.toLocaleString()} GB/month, use Cloud CDN cache policies, compression, and billing alerts to control internet egress.`);
  }

  // 9. Assumptions
  const sizingNotes = [
    `Sizing baseline: ${normalizedExpectedUsers.toLocaleString()} expected monthly users and ${normalizedConcurrentUsers.toLocaleString()} concurrent users.`,
    isHighConcurrency
      ? 'Sizing posture: high concurrency requires autoscaling compute, load testing, and explicit maximum-capacity guardrails.'
      : 'Sizing posture: moderate concurrency can start with managed autoscaling and be tuned from production telemetry.',
    `Transfer baseline: ${normalizedDataTransfer.toLocaleString()} GB/month${isHighTransfer ? ', so edge caching and egress monitoring are recommended.' : ', suitable for standard transfer monitoring.'}`
  ];
  const assumptions = [
  ...
  if (isHighConcurrency) {
    risks.push(`**Capacity Validation:** The estimate of ${normalizedConcurrentUsers.toLocaleString()} concurrent users requires load testing; pod, instance, database connection, and quota limits may otherwise become bottlenecks.`);
  }
  if (isVeryHighTransfer) {
    risks.push(`**High Network Egress Cost:** Approximately ${normalizedDataTransfer.toLocaleString()} GB/month can create material egress charges if CDN cache-hit ratios and traffic destinations are not monitored.`);
  }
  ...
  } else if (computeService.name.includes('Cloud Run') && isMultiRegion) {
    activeNodes.ComputeA = 'ComputeA[Cloud Run Service: Region A]:::compute';
    activeNodes.ComputeB = 'ComputeB[Cloud Run Service: Region B]:::compute';
  } else if (computeService.name.includes('Multi-Region Managed Instance Groups')) {
    activeNodes.ComputeA = 'ComputeA[Compute Engine MIG: Region A]:::compute';
    activeNodes.ComputeB = 'ComputeB[Compute Engine MIG: Region B]:::compute';
