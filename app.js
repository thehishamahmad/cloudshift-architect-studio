/**
 * CloudShift AI Architect - Front-end App Controller (Version 1.2)
 * Coordinates inputs, local history, Mermaid rendering, and Markdown downloads
 * for the Cloud Architecture Assessment Platform.
 */
...
// Outputs: Recovery & DR (Version 1.2)
...
function getFormInputs() {
  ...
  const expectedUsers = parseInt(document.getElementById('input-expected-users').value) || 50000;
  const concurrentUsers = parseInt(document.getElementById('input-concurrent-users').value) || 1000;
  const dataTransfer = parseInt(document.getElementById('input-data-transfer').value) || 2500;
  const recoveryTier = document.getElementById('input-recovery-tier').value;
  const dailyChangeRate = parseInt(document.getElementById('input-daily-change-rate').value) || 5;
  
  // Retentions
  const retainDaily = parseInt(document.getElementById('retain-daily').value) || 14;
  const retainWeekly = parseInt(document.getElementById('retain-weekly').value) || 4;
  const retainMonthly = parseInt(document.getElementById('retain-monthly').value) || 12;
  const retainYearly = parseInt(document.getElementById('retain-yearly').value) || 1;
...
function syncFormInputs(inputs) {
  ...
  document.getElementById('input-concurrent-users').value = inputs.concurrentUsers;
...
function populateDashboard(rec) {
  ...
  // 2. Core Narrative Summary
  outputSummary.innerHTML = rec.summary;
  ...
  rec.services.forEach(svc => {
    ...
    card.innerHTML = `
      <div class="service-icon-box ${catClass}">
        <i data-lucide="${svc.icon}" style="width: 22px; height: 22px;"></i>
      </div>
      <div class="service-info">
        <div class="service-name-row">
          <span class="service-name">${svc.name}</span>
          <span class="service-category ${catClass}">${svc.category}</span>
        </div>
        <p class="service-description">${svc.rationale}</p>
      </div>
    `;
  ...
  if (rec.risks.length === 0) {
    ...
    rec.risks.forEach(risk => {
      ...
      item.innerHTML = `
        <i data-lucide="${iconName}" class="risk-item-icon" style="width: 16px; height: 16px;"></i>
        <span>${risk}</span>
      `;
  ...
  // 8. Recovery & DR Design (Version 1.2)
  rtoValue.textContent = rec.rtoValue;
  rpoValue.textContent = rec.rpoValue;
  outputDRDesign.innerHTML = rec.recoveryDR;
...
async function renderMermaidDiagram(code) {
  ...
  wrapper.innerHTML = `<div class="mermaid" id="mermaid-graph">${code}</div>`;
    ...
    try {
      ...
      wrapper.innerHTML = `
        <div style="color:var(--color-danger); padding:20px; text-align:center;">
          ...
          <p>Failed to render graphic: ${err.message}</p>
        </div>
      `;
...
function exportMarkdownDocument() {
  ...
  let md = `# Cloud Architecture Assessment: ${rec.projectName}\n\n`;
  ...
  md += `*Generated by CloudShift AI Architect*\n\n`;
  md += `---\n\n`;

  // 1. Executive Summary
  md += `# Executive Summary\n\n`;
  md += `### Business Outcome\n${rec.execSummary.businessOutcome}\n\n`;
  ...
  // 4. Recovery & Disaster Recovery Design
  ...
  history.forEach(item => {
    ...
    card.innerHTML = `
      <div class="history-header">
        <span class="history-title" title="${item.projectName}">${item.projectName}</span>
        <span class="history-time">${item.timestamp}</span>
      </div>
      <div class="history-details">${item.workloadList} • ${availLabel} • Budget: ${budgetLabel}</div>
      <button class="history-delete-btn" title="Delete Saved Assessment">
