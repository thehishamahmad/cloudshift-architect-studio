...
(function attachCloudShiftHybridMermaid(global) {
  ...
  function addDependencyEdge(edges, from, to, label) {
    const edge = `  ${from} -.->|${label}| ${to}`;
    if (!edges.includes(edge)) edges.push(edge);
  }
  ...
  function generateHybridMermaidDiagram(inputs = {}, recommendation = {}, hybridRecommendation = null, hybridServices = null) {
    ...
    const includeSecretManager = hasService(hybridServices, 'security', 'Secret Manager');
    ...
    if (mixedAccess) {
      addNode(nodes, 'PublicUser', 'PublicUser([Public Users]):::client');
      addNode(nodes, 'InternalUser', 'InternalUser([Corporate / Internal Users]):::client');
    } else {
      addNode(nodes, 'User', hasInternalAccess ? 'User([Corporate / Internal Users]):::client' : 'User([User Clients / Browser]):::client');
    }
    if (includeLoadBalancer || includeCdn || includeArmor) addNode(networkNodes, 'GCLB', 'GCLB[Global HTTPS Load Balancer]:::gateway');
    ...
    if (includeArmor) addNode(securityNodes, 'Armor', 'Armor[Cloud Armor WAF]:::sec');
    if (includeSecretManager) addNode(securityNodes, 'SecMgr', 'SecMgr[Secret Manager]:::sec');
    ...
    let ingressNode = networkNodes.has('GCLB') ? 'GCLB' : mixedAccess ? 'PublicUser' : 'User';
    if (networkNodes.has('GCLB')) addEdge(edges, mixedAccess ? 'PublicUser' : 'User', 'GCLB');
    if (includeCdn && networkNodes.has('GCLB')) {
      addEdge(edges, 'GCLB', 'CDN');
      ingressNode = 'CDN';
    }
    if (includeArmor) {
      addEdge(edges, ingressNode, 'Armor');
      ingressNode = 'Armor';
    }
    if (includeCloudVpn) addEdge(edges, 'VPN', 'GCLB');
    if (includePrivateServiceConnect) addEdge(edges, 'PSC', 'GCLB');
    ...
    ['CloudRun', 'Compute', 'GKE'].forEach(node => {
      if (includeSecretManager && appNodes.has(node)) addDependencyEdge(edges, node, 'SecMgr', 'reads secrets');
    });
    ...
    nodes.forEach(definition => { mermaid += `    ${definition}\n`; });
    mermaid += '  end\n\n';
    if (securityNodes.size > 0) {
      mermaid += '  subgraph Security ["Security Layer"]\n';
      securityNodes.forEach(definition => { mermaid += `    ${definition}\n`; });
      mermaid += '  end\n\n';
    }
    ...
    mermaid += '  subgraph App ["Application Lanes"]\n';
    appNodes.forEach(definition => { mermaid += `    ${definition}\n`; });
    mermaid += '  end\n\n';
    mermaid += '  subgraph Data ["Data & Analytics Layer"]\n';
